...
from CogModule.aca_scienceworld_skeleton import *


def randomModel(args, taskid, variation=-1, if_raw=False, if_sleep=True, if_event=True):
    """ Example random agent -- randomly picks an action at each step. """
    exitCommands = ["quit", "exit"]

    taskIdx = taskid#args['task_num']
    simplificationStr = args['simplification_str']
    numEpisodes = 1#args['num_episodes']
    ...
    for episodeIdx in range(0, numEpisodes):
        ...
        if variation == -1:
            variation = env.get_random_variation_train()
        env.load(taskName, variation, simplificationStr)
        ...
        print("Task Variation: " + str(variation) + " / " + str(maxVariations))
        ...
        aca_agent = ACAAgent(str(env.get_task_description()))
        llm_com = LLMCommunicator(None)
        aca_agent.llm_communicator = llm_com
        if not if_sleep:
            llm_com.sleep_step = 300
        if not if_event:
            llm_com.if_event = False

        validActions = env.getValidActionObjectCombinations()#env.get_valid_action_object_combinations_with_templates()    
        with open("actions.txt", "a") as f:
            f.writelines(str(validActions))
        for path in [sm_path, cm_path, pm_path]:
            with open(path, 'w') as f:
                f.write("")

        while (userInputStr not in exitCommands) and (isCompleted is False) and curIter < 21:
            print("----------------------------------------------------------------")
            print("Step: " + str(curIter))
            print("userInputStr: " + userInputStr)
            ...
            llm_com.record_basic(taskIdx, score, reward, userInputStr, curIter, if_raw=if_raw, variation=variation)     
            ...
            aca_agent.pm.set_actions(validActions, aca_agent.get_all_obs(obs=observation))
            userInputStr = llm_com.get_action_and_update(aca_agent.build_prompt(obs=observation), aca_agent, observation, if_raw=if_raw, taskNum=taskIdx, variation=variation)[1]#randomAction["action"]#input("input:")#
            print("Next action: " + userInputStr)
...
def main():
    ...
    for i in [2, 5, 12, 18, 20, 23, 25, 28]:
            for b in range(2):
                raw = b != 0
                randomModel(args, i, 0, raw)
                if not raw:
                    randomModel(args, i, 0, raw, False)
                    randomModel(args, i, 0, raw, True, False)
