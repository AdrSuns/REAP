...
```json
{
  ...
  "new_semantic_memory"(max item num of 3): [
    {
      "name": "rule name",
      "conditions": {"object.attribute_change": "value"},
      "precondition": {"object.attribute": "value"},
      "effects": {"object.attribute_change": "new_value"},
      "confidence": 0.95
    }
  ],
  "new_procedural_memory"(max item num of 3): [
    {
      ...
      "target": "target name",
      "precondition": {"object.attribute": "value"},
      "effect": {"object.attribute_change": "new_value"},
      "confidence": 0.9
    }
  ]
}
...
### Working Memory (WM) - What you focus on
...
- Be careful on decision of 'focus', you will instantly fail if you focus on an wrong object, but if the object is the very key object, you will instantly win.
...
3. **(Optional) Summarize new generalizable rules (semantic and procedural)** from recent experience.  
...
```json
{
  ...
  "action": "<fill the action_value to the action template>",  
  "new_working_memory"(max item num of 3): [    
    {
      "name": "object name",
      "attributes": {"0": <attribute0>, "1": <attribute1>, ...}
    }
...
analyse_system_message_no_event = """
You are an advanced cognitive agent operating in an embodied reasoning environment.
...
**Select the next action** to achieve the goal. Choose ONLY from the provided Available Actions list. Your job is to choose one valid action at each step from the given **Action Templates** and corresponding **Available Objects**. You must strictly follow these constraints:
