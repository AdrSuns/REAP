...
#CUDA_VISIBLE_DEVICES=0,1,2,3 python main.py --task_num 0 --env_step_limit 100 --lm_path sciworld_11b_rerun_dt --simplification_str easy --beams 16 --max_episode_per_file 1000 --mode dt --set test --output_path logs --model_parallelism_size 4
...
#
def findValidActions(predictions, env, lastActions):
...
def T5Model(args):
    ...
    tokenizer = T5Tokenizer.from_pretrained(args["lm_path"], low_cpu_mem_usage=True)
    ...
    if (len(args["output_path"]) > 0):
        ...
        txt_path = args['output_path'] + "task_" + str(args['task_num']) + ".txt"
    ...
    for variation in [0]:
        ...
        #aca_agent = reap.ACAAgent(taskName)
        ...
        while not done:
            ...
            if is_t5_raw:
                ...
                aca_agent.em.history.append(obs)
                print(obs)
                aca_agent.pm.set_actions(env.getValidActionObjectCombinations(), aca_agent.get_all_obs(obs))
                input_str =  analyse_system_message + "\n" + aca_agent.build_prompt(obs=obs)
            ...
            print("sample_outputs: " + str(sample_outputs))
            ...
            res_text = f"Variation: {variation}, Step: {step}, Score: {score}, Action: {action}"
            print(res_text)
